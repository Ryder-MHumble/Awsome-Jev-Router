...
[![本地优先](https://img.shields.io/badge/Skill-本地优先-16a34a)](skill/jev-practice-recommender/)
[![License](https://img.shields.io/badge/license-MIT-yellow.svg)](LICENSE)

[**中文**](README.zh-CN.md) · [English](README.md) · [目录](#目录) · [贡献](CONTRIBUTING.md)

Awsome-Jev-Router 是一个可检索的 Jev 实践目录和本地 Agent skill。它从脱敏日志中识别重复决策点，并关联公开项目、实现说明和来源证据，帮助你决定先验证什么；它不会运行第三方项目，也不会自动修改 Agent。

## 30 秒了解

### Jev 是什么？

Jev 是 TypeSafe AI 推出的首个 **System One Model**，面向软件可直接使用的快速、结构化决策。输入状态和类型化问题后，它返回 `Choice`、`Score` 或 `Noul` 等受约束结果，而不是一段长聊天回复。

设计、API 示例和基准声明见[官方发布文章](https://typesafe.ai/blog/introducing-system-one-models-and-jev)。Jev 仍处于 early access；文章中的性能数字是 TypeSafe 的声明，不是本仓库独立测得的基准。

### Jev 为什么受到关注？

很多 Agent 工作流需要的是小而重复的判断，而不是再次调用长文本生成。Jev 的 early access 发布、公开 SDK 和 skill 生态，以及围绕路由、工具安全、上下文压缩和浏览器动作的社区实验，让这种思路容易上手和讨论。

受到关注不代表它适合所有工作流。应先验证：这个判断是否稳定到可以类型化、是否足够便宜以便频繁评估，以及低置信度时是否能安全交给人复核。

| 长文本 LLM 调用 | Jev 式决策点 |
| --- | --- |
| 生成文本或代码，由调用方解析和校验 | 返回受约束的值，调用方可直接分支 |
| 适合开放式推理和综合 | 适合工作流中的重复、边界明确的选择 |
| 输出本身是文本时更合适 | 选择范围窄时可减少通用模型调用 |

```text
Agent 日志 → 本地脱敏 → 高频场景 → Jev 实践候选 → 来源与证据
```

项目把阈值、升级和副作用控制留在业务代码中：推荐是待复核线索，不是自动执行。
